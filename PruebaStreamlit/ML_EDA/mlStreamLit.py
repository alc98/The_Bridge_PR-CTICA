...
def load_data() -> pd.DataFrame:
    ...
    # Ruta única específica
    path = base_dir / "PruebaStreamlit" / "ML_EDA" / "housing_price_dataset.csv (1).zip"

    if path.exists():
        st.info(f"📄 Cargando dataset desde: {path}")
        # Es un ZIP, lo leemos con compresión
        return pd.read_csv(path, compression="zip")

    # Si no se encuentra en disco, se pide subirlo manualmente
    st.warning(
        "⚠️ No se ha encontrado el archivo de datos en la ruta esperada:\n\n"
        f"`{path}`\n\n"
